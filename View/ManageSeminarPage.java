...
public class ManageSeminarPage extends JPanel {
    ...
    public ManageSeminarPage(MainFrame mainFrame) {
        ...
        sessionButton.addActionListener(e -> {
            ...
            // Create a dummy session object to pass to readSessions 
            final Session tempSession = new Session(sessionSem);

            JDialog viewDialog = new JDialog((Frame) SwingUtilities.getWindowAncestor(this), "Sessions for: " + seminarTitle, true);
            viewDialog.setSize(800, 400);
            viewDialog.setLayout(new BorderLayout());

            String[] columns = { "Session ID", "Type", "Start Time", "End Time", "Evaluator" };
            ...
            addSessionBtn.addActionListener(ev -> {
                ...
                int newSessionID =  (currentSessList.size() + 1);
                ...
                // Correct Constructor Order: (Seminar, ID, Type, Start, End)
            ...
            delSessionBtn.addActionListener(ev -> {
               int selectedSessionRow = sessionTable.getSelectedRow();
                ...
                int sessID = (int) sessionModel.getValueAt(selectedSessionRow, 0);
                csvModel.deleteSession(sessID);
